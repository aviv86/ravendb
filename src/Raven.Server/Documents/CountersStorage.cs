...
;
        ...
        public string DeleteCounter(DocumentsOperationContext context, string documentId, string collection, string counterName, bool forceTombstone = false, long lastModifiedTicks = -1)
        {
            ...
            using (DocumentIdWorker.GetLowerIdSliceAndStorageKey(context, documentId, out Slice lowerId, out _))
            {
                ...
                BlittableJsonReaderObject.RawBlob counterToDelete = null;
                if (data.TryGet(Counters, out BlittableJsonReaderObject counters) == false ||
                    counters.TryGet(counterName, out counterToDelete) == false)
                {
                ...
                {
                    RemoveCounterFromBlittableAndUpdateTable(context, counterName, data, counters, collectionName, table, lowerId);
                }
