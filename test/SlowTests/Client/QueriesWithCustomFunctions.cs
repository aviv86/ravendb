            ...
            using (var store = GetDocumentStore())
            {
                ...
                using (var session = store.OpenSession())
                {
                    var query = from user in session.Query<User>()
                        ...
                        let groups = RavenQuery.Load<Group>(user.Groups)
                        select new {configuration.Language};
            ...
            using (var store = GetDocumentStore())
            {
                ...
                using (var session = store.OpenSession())
                {
                    var query = from u in session.Query<User>()
                        ...
                        select new {u.Name, Detail = detail};
        ...
        public void Can_Project_With_JsonPropertyAttribute()
        {
            ...
            using (var session = store.OpenSession())
            {
                ...
                var projection =
                    ...
                    {
                        s.Id,
                        Results = s.Results.Select(x => new {x.ResultValue})
                            .ToArray()
                    };
            ...
            using (var store = GetDocumentStore())
            {
                ...
                using (var session = store.OpenSession())
                {
                    var query = from u in session.Query<UserGroup>()
                        select new {u.Name, UsersByName = u.Users.ToDictionary(a => a.Name), UsersByNameLastName = u.Users.ToDictionary(a => a.Name, a => a.LastName)};
...
from Users as u load u.FriendId as _doc_0 select output(u, _doc_0)", query.ToString());
        ...
        public void Custom_Function_With_GetMetadataFor()
        {
            using (var store = GetDocumentStore())
            {
                ...
                {
                    var query = from u in session.Query<User>()
                        select new {u.Name, Metadata = session.Advanced.GetMetadataFor(u)};
...
from Users as u where u.Name != $p0 select output(u)",
        ...
        public void Custom_Function_With_RavenQueryMetadata()
        {
            using (var store = GetDocumentStore())
            {
                ...
                {
                    var query = from u in session.Query<User>()
                        select new {u.Name, Metadata = RavenQuery.Metadata(u)};
            ...
            using (var store = GetDocumentStore())
            {
                ...
                using (var session = store.OpenSession())
                {
                    var complexLinqQuery =
                        ...
                        select new {o.Id, TotalMoneySpent = TotalSpentOnOrder(o)};
        ...
        public void Custom_Functions_Escape_Hatch_With_Path()
        {
            using (var store = GetDocumentStore())
            {
                ...
                using (var session = store.OpenSession())
                {
                    var query = session.Query<User>()
                        .Where(u => u.Name == "Jerry")
                        .Select(a => new {Name = RavenQuery.Raw(a.Name, "substr(0,3)")});
        ...
        public void Custom_Functions_Linq_Methods_Support()
        {
            using (var store = GetDocumentStore())
            {
                var user = new User
                {
                    ...
                    Details = new List<Detail> {new Detail {Number = 19}, new Detail {Number = -25}, new Detail {Number = 27}, new Detail {Number = 6}}
                };
                ...
                using (var session = store.OpenSession())
                {
                    var query = from u in session.Query<User>()
                        select new
                        {
                            ...
                            SplitLimit = u.Name.Split(new[] {'r'}, 3),
                            SplitArray = u.Name.Split('r', 'e'),
                            SplitArgument = u.Name.Split(u.Roles, StringSplitOptions.None),
                            SplitStringArray = u.Name.Split(new[] {"er", "rr"}, StringSplitOptions.None),
                    ...
                    Assert.Equal("Jerry".Split('r'), queryResult[0].Split);
                    Assert.Equal("Jerry".Split(new[] {'r'}, 3), queryResult[0].SplitLimit);
                    Assert.Equal("Jerry".Split('r', 'e'), queryResult[0].SplitArray);
                    Assert.Equal("Jerry".Split(new[] {"er", "rr"}, StringSplitOptions.None), queryResult[0].SplitStringArray);
                    ...
                    Assert.Equal("Bob".Split(new[] {'o'}), queryResult[1].SplitArgument);
            ...
            using (var store = GetDocumentStore())
            {
                ...
                using (var session = store.OpenSession())
                {
                    var query = from o in session.Query<Order>()
                        ...
                        select new
                        {
                            Company = company.Name, Employee = employee.FirstName + " " + employee.LastName, Manager = manager.FirstName + " " + manager.LastName
                        };
        ...
        public void Custom_Functions_With_SelectMany()
        {
            using (var store = GetDocumentStore())
            {
                var nestedNode = new Node
                {
                    ...
                    {
                        Name = "Child" + x, Children = Enumerable.Range(0, 5).Select(y => new Node {Name = "Grandchild" + (x * 5 + y), Children = null}).ToList()
                    }).ToList()
                };
                ...
                using (var session = store.OpenSession())
                {
                    ...
                    for (var i = 0; i < 50; i++) Assert.Equal("Grandchild" + i, queryResult[0].Grandchildren[i].Name);
                }
            }
        }
            ...
            using (var store = GetDocumentStore())
            {
                ...
                using (var session = store.OpenSession())
                {
                    var query = from u in session.Query<User>("UsersByNameAndFriendId")
                        where u.Name != "Pigpen"
                        let friend = RavenQuery.Load<User>(u.FriendId)
                        select new {u.Name, Friend = friend.Name};
            ...
            using (var store = GetDocumentStore())
            {
                ...
                using (var session = store.OpenSession())
                {
                    var complexLinqQuery = from o in session.Query<Order>()
                        ...
                        let employee = session.Load<Employee>(o.Employee)
                        select new {o.Id, Status = "Ordered at " + o.OrderedAt + ", by " + employee.FirstName + " " + employee.LastName};
